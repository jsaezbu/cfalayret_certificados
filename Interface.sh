...
CONTRACT_ADDRESS="erd1..." # Sustituye por la dirección real de tu smart contract
PEM_PATH="$HOME/mi_wallet.pem" # Ruta al archivo PEM de la wallet del emisor
PROXY="https://devnet-gateway.multiversx.com"
CHAIN="D"

# FUNCIÓN: Expedir un nuevo certificado
expedir_certificado() {
  echo "Introduce el hash (huella) del certificado PDF:"
  read hash_certificado
  echo "Introduce la dirección de wallet del alumno (erd1...):"
  ...
  mxpy contract call $CONTRACT_ADDRESS \
    ...
    --arguments hex:$(echo -n "$hash_certificado" | xxd -p -c 256) $direccion_alumno \
    --gas-limit 50000000 \
    --send
...
comprobar_certificado() {
  ...
  mxpy contract query $CONTRACT_ADDRESS \
    ...
    --arguments hex:$(echo -n "$hash_certificado" | xxd -p -c 256) \
    --proxy $PROXY \
    --chain $CHAIN
}
